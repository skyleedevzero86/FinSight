...
public class RateLimitConfig {
        ...
        public Bucket createBucket() {
                logger.info("로컬 메모리 기반 요청 한도 버킷 생성");
        ...
        public ProxyManager<byte[]> proxyManager(StatefulRedisConnection<byte[], byte[]> connection) {
                try {
                        logger.info("Redis 프록시 매니저 생성");
                        return LettuceBasedProxyManager.builderFor(connection).build();
                } catch (Exception e) {
                        logger.error("Redis 프록시 매니저 생성 실패: {}", e.getMessage());
                        throw new RuntimeException("Redis 프록시 매니저 생성 실패", e);
                }
        }
                ...
                try {
                        BucketConfiguration configuration = BucketConfiguration.builder()
                                        .addLimit(limit -> limit.capacity(defaultRequestsPerMinute)
                                                        .refillGreedy(defaultRequestsPerMinute, Duration.ofMinutes(1)))
                                        .build();
                        logger.info("분산 요청 한도 버킷 생성");
                        return proxyManager.builder().build("default-bucket".getBytes(), configuration);
                } catch (Exception e) {
                        logger.error("분산 요청 한도 버킷 생성 실패: {}", e.getMessage());
                        throw new RuntimeException("분산 요청 한도 버킷 생성 실패", e);
                }
        }
        ...
        public Bucket createDistributedStrictBucket(ProxyManager<byte[]> proxyManager) {
                try {
                        ...
                        logger.error("분산 엄격 요청 한도 버킷 생성 실패: {}", e.getMessage());
                        throw new RuntimeException("분산 엄격 요청 한도 버킷 생성 실패", e);
                }
        }
        ...
        public Bucket createDistributedAdminBucket(ProxyManager<byte[]> proxyManager) {
                try {
                        ...
                        logger.error("분산 관리자 요청 한도 버킷 생성 실패: {}", e.getMessage());
                        throw new RuntimeException("분산 관리자 요청 한도 버킷 생성 실패", e);
                }
        }
}