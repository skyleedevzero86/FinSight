...
@Tag(
        name = "메인이미지 CMS",
        description = "메인 히어로 이미지 CRUD. 공개 노출 목록(GET, reflectOnly=true)은 비인증 가능. 등록·수정·삭제·전체목록은 ADMIN/MANAGER.")
@RestController
...
public class MainimgItemController {
    ...
    @Operation(
            summary = "메인이미지 목록 조회",
            description = "reflectOnly=true(기본): 화면 반영(Y) 항목만 공개 조회. reflectOnly=false: ADMIN/MANAGER만 전체 조회.")
    @GetMapping
    @PreAuthorize("#reflectOnly == true or hasAnyRole('ADMIN','MANAGER')")
    public ResponseEntity<ApiResponse<PaginationResponse<MainimgItemResponse>>> list(
            @Parameter(description = "사이트·테넌트 구분 (선택)")
            @RequestParam(required = false) String domainId,
            @Parameter(description = "true면 노출(Y)만, false면 전체(관리자)")
    ...
    public ResponseEntity<ApiResponse<MainimgItemResponse>> update(
        ...
        log.info("메인이미지 수정 - id={}", id);
        return ResponseEntity.ok(ApiResponse.success(mainimgItemService.update(id, request), "메인이미지를 수정했습니다"));
    }

    @Operation(summary = "메인이미지 삭제", description = "메인이미지 항목 삭제. ADMIN/MANAGER.")
    @SecurityRequirement(name = "BearerAuth")
    @DeleteMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    public ResponseEntity<ApiResponse<Void>> delete(@PathVariable String id) {
        log.info("메인이미지 삭제 - id={}", id);
