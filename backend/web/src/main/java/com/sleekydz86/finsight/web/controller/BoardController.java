...
public class BoardController {
    ...
    public ResponseEntity<ApiResponse<List<BoardListResponse>>> getPopularBoards(
        ...
        try {
            if (limit <= 0 || limit > 100) {
                throw new ValidationException("제한 수는 1-100 사이여야 합니다");
            }
    ...
    private void validateSearchRequest(BoardSearchRequest request) {
        if (request == null) {
            throw new ValidationException("검색 요청이 null입니다");
        }
    ...
    private void validateCreateRequest(BoardCreateRequest request) {
        if (request == null) {
            throw new ValidationException("생성 요청이 null입니다");
        }
        ...
        if (request.getTitle().length() > 200) {
            throw new ValidationException("제목은 200자를 초과할 수 없습니다");
        }
        if (request.getContent().length() > 10000) {
            throw new ValidationException("내용은 10000자를 초과할 수 없습니다");
        }
    }

    private void validateUpdateRequest(BoardUpdateRequest request) {
        if (request == null) {
            throw new ValidationException("수정 요청이 null입니다");
        }
        if (request.getTitle() == null || request.getTitle().trim().isEmpty()) {
            throw new ValidationException("제목은 필수입니다");
        }
    ...
    private void validateReportRequest(BoardReportRequest request) {
        if (request == null) {
            throw new ValidationException("신고 요청이 null입니다");
        }
